            ...
            <div
                ...
                <form method="POST" action="{{ route('admin.courses.update', $course) }}" enctype="multipart/form-data">
                    ...
                    <div>
                        ...
                        <x-text-input id="name" class="block mt-1 w-full transition-all duration-300 ease-in-out"
                            type="text" name="name" value="{{ $course->name }}" required autofocus
                            autocomplete="name" />
                        <x-input-error :messages="$errors->get('name')" class="mt-2" />
                    </div>

                    <div class="mt-4">
                        <x-input-label for="thumbnail" :value="__('thumbnail')" class="text-gray-700 dark:text-gray-300" />
                        <img src="{{ Storage::url($course->thumbnail) }}" alt=""
                            class="rounded-2xl object-cover w-[120px] h-[90px]">
                        <x-text-input id="thumbnail" class="block mt-1 w-full transition-all duration-300 ease-in-out"
                            type="file" name="thumbnail" autofocus autocomplete="thumbnail" />
                        <x-input-error :messages="$errors->get('thumbnail')" class="mt-2" />
                    </div>

                    {{-- path trailer ini code unique youtube --}}
                    <div class="mt-4">
                        <x-input-label for="path_trailer" :value="__('path_trailer')" class="text-gray-700 dark:text-gray-300" />
                        <x-text-input id="path_trailer"
                            class="block mt-1 w-full transition-all duration-300 ease-in-out" type="text"
                            name="path_trailer" value="{{ $course->path_trailer }}" required autofocus
                            autocomplete="path_trailer" />
                        <x-input-error :messages="$errors->get('path_trailer')" class="mt-2" />
                    </div>

                    <div class="mt-4">
                        <x-input-label for="category" :value="__('category')" class="text-gray-700 dark:text-gray-300" />
                    ...
                    <div class="mt-4">
                        ...
                        <textarea name="about" id="about" cols="30" rows="5" class="border border-slate-300 rounded-xl w-full">{{ $course->about }}
                        </textarea>
